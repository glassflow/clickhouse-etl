...
| **Observability (OTEL)**          | OTEL-first structured logging and metrics. `structuredLogger` replaces all `console.*` calls. Env vars injected at runtime via `window.__ENV__`. | Adding OTEL vars requires updating `startup.sh`, `generate-env.mjs`, `next.config.ts`, and `config.ts`. Do not bypass `structuredLogger` with raw `console.*`. | [docs/implementations/OBSERVABILITY_IMPLEMENTATION.md](../../docs/implementations/OBSERVABILITY_IMPLEMENTATION.md) |
