...
export async function POST(request: Request) {
  if (isMockMode()) {
    const topicDetails = generateMockKafkaTopics().map((name, i) => ({
      name,
      partitionCount: 1 + (i % 6),
    }))
    return NextResponse.json({ success: true, topicDetails })
  }
  ...
  try {
    ...
    try {
      ...
      structuredLogger.error('Error fetching Kafka topic details', {
    ...
    structuredLogger.error('Error processing Kafka topic-details request', {
      error: error instanceof Error ? error.message : String(error),
    })
    return NextResponse.json(
      {
