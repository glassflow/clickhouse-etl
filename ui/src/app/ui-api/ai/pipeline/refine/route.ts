...
/**
 * POST /ui-api/ai/pipeline/refine
 * Refines an existing intent based on follow-up user answers.
 * Re-runs connection enrichment when passwords are provided so the user
 * can supply credentials in the right panel and have them tested immediately.
 */
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const { userMessage, intent, messages, kafkaPassword, clickhousePassword } = parsed.data as IntentApiRequest

    // ── 1. Re-test Kafka if password provided or connection not yet valid ──
    let kafkaEnrichment: {
    ...
    if (canTestKafka) {
      try {
        ...
        structuredLogger.warn('Kafka enrichment failed during AI refine', {
    ...
    // ── 2. Re-test ClickHouse if connection not yet valid ──
    ...
    if (intent?.clickhouse?.host && intent.clickhouse.connectionStatus !== 'valid') {
      try {
        ...
        structuredLogger.warn('ClickHouse enrichment failed during AI refine', {
          error: err instanceof Error ? err.message : String(err),
        })
        clickhouseEnrichment = {
    ...
    // ── 3. Merge enrichment into intent for LLM context ──
    ...
    structuredLogger.error('AI refine route error', {
