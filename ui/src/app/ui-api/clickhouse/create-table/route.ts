...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    if (!database || !table || !engine || !order_by || !Array.isArray(columns) || columns.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: 'database, table, engine, order_by, and columns (non-empty) are required',
        },
        { status: 400 },
      )
    }
    ...
    const query = buildCreateTableQuery(
      ...
      columns.map((c: any) => ({
        ...
        type: (c.type || c.column_type || 'String').replace(/^Nullable\((.*)\)$/, '$1'),
        isNullable: c.isNullable ?? (c.type || '').includes('Nullable'),
      })),
    )
    ...
    return NextResponse.json({
      success: true,
      message: `Table ${database}.${table} created successfully`,
    })
  } catch (error) {
    return NextResponse.json(
      {
