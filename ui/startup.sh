...
echo ""
echo "NEXT_PUBLIC_AI_ENABLED: $NEXT_PUBLIC_AI_ENABLED"
echo ""
echo "=== Server-Side AI/LLM Variables (Before Export) ==="
...
# AI feature flag (client-visible)
export NEXT_PUBLIC_AI_ENABLED=${NEXT_PUBLIC_AI_ENABLED:-true}

# Server-side only LLM variables — not prefixed with NEXT_PUBLIC_, never inlined by Next.js
export LLM_PROVIDER=${LLM_PROVIDER:-openai}
export OPENAI_MODEL=${OPENAI_MODEL:-gpt-4o}
export ANTHROPIC_MODEL=${ANTHROPIC_MODEL:-claude-sonnet-4-6}
# API keys: export as passthrough with empty default (no value printed in logs)
...
echo "  NEXT_PUBLIC_AI_ENABLED: \"$NEXT_PUBLIC_AI_ENABLED\"," >> /app/public/env.js
...
echo "NEXT_PUBLIC_AI_ENABLED: $NEXT_PUBLIC_AI_ENABLED"
echo ""
echo "=== Server-Side AI/LLM Variables (After Export) ==="
echo "LLM_PROVIDER: $LLM_PROVIDER"
echo "OPENAI_MODEL: $OPENAI_MODEL"
echo "ANTHROPIC_MODEL: $ANTHROPIC_MODEL"
echo "OPENAI_API_KEY: ${OPENAI_API_KEY:+set}"
