...
func TestFilter_Satisfies(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := filter.Satisfies([]byte(tt.jsonData))
			if (err != nil) != tt.wantErr {
				t.Errorf("Filter.Satisfies() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("Filter.Satisfies() = %v, want %v", got, tt.want)
			}
		})
	}
}
