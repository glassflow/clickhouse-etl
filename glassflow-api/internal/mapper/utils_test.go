...
func TestParseUint8(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "Valid uint16 within range",
			input:       uint16(100),
			expected:    100,
			expectError: false,
		},
		...
		{
			name:        "Valid uint32 within range",
			input:       uint32(200),
			expected:    200,
			expectError: false,
		},
...
func TestParseInt8(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "Int16 too small",
			input:       int16(math.MinInt8 - 1),
			expectError: true,
		},
		{
			name:        "Valid int32 within range",
			input:       int32(42),
			expected:    42,
			expectError: false,
		},
		...
		{
			name:        "Int64 negative within range",
			input:       int64(-100),
			expected:    -100,
			expectError: false,
		},
