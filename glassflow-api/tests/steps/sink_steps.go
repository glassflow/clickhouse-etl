...
func (s *SinkTestSuite) natsStreamSubjectHasNEvents(natsStream, natsSubject string, expectedCount int) error {
	time.Sleep(1 * time.Second) // Give some time for messages to be published to DLQ
	...
	msgs, err := consumer.Fetch(expectedCount*2, jetstream.FetchMaxWait(fetchTimeout))
	if err != nil {
		return fmt.Errorf("fetch messages from DLQ: %w", err)
	}
