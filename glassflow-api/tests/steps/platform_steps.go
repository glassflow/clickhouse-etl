...
func (p *PlatformSteps) setupServices() error {
	...
	p.pipelineService = service.NewPipelineService(p.orchestrator, db, p.log, trackingClient)

	// Create HTTP router
	p.httpRouter = api.NewRouter(p.log, p.pipelineService, nil, nil, trackingClient)
