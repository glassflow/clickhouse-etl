---
title: 'Migration Guide: v2.4.x to v2.5.x'
description: 'Complete guide for migrating from GlassFlow v2.4.x to v2.5.x with PostgreSQL metadata storage'
---
...
<Callout type="info">
This guide targets the v2.4.x to v2.5.x upgrade. The commands, ports, and API endpoints shown reflect the v2-era API and differ from later versions. Follow the guide that matches the version you are upgrading from.
</Callout>
...
glassflow/glassflow-etl 0.4.1          2.5.1           A Helm chart for Kubernetes
```
...
# PostgreSQL configuration (new in v2.5.x)
postgresql:
  enabled: true
  auth:
    database: "glassflow"
