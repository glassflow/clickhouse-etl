...
description: 'Migrate from GlassFlow v2.10.x to v2.11.x when using chart-managed encryption. Use chart v0.5.12 to fix encryption secret handling.'
---
...
Save the script above as `create-target-secret.sh` and make it executable:

```bash
chmod +x create-target-secret.sh
```
...
Run the Helm upgrade using chart version **0.5.12** and set encryption to use the secret you created in step 1 (replace `<secret-name>` with the name you chose in step 1, e.g. `glassflow-encryption-key-external`):

```bash
helm upgrade <release_name> <chart_reference> -n <namespace> --version 0.5.12 \
...
```bash
helm upgrade glassflow glassflow/glassflow-etl -n glassflow --version 0.5.12 \
  --set global.encryption.enabled=true \
