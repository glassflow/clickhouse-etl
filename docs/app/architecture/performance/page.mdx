...
You can scale in two ways: **within a pipeline** (multiple replicas of ingestor, transform, and sink) and **across pipelines** (adding more pipelines). For component-level scaling and best practices, see [Scaling Pipelines](/usage-guide/scaling).

Because GlassFlow pipelines operate independently, throughput also scales horizontally by adding pipelines.
