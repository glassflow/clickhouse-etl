...
1. **Source Acknowledgment**: The source-specific mechanism (Kafka offset commits or OTLP HTTP/gRPC response codes) confirms ingestion only after successful processing.
...
2. Each message is published to a NATS JetStream stream.
...
1. The shared [OTLP Receiver](/architecture/kubernetes-components#otlp-receiver) accepts OpenTelemetry data over gRPC (port 4317) or HTTP (port 4318).
2. The receiver flattens the OTLP protobuf payload into a JSON structure and routes it to the correct pipeline using the `x-glassflow-pipeline-id` header.
3. Flattened messages are published to NATS JetStream.
...
The OTLP receiver is a shared component that does not check pipeline status. When an OTLP pipeline is **stopped**, the receiver continues accepting incoming data and publishing it to NATS JetStream. Messages accumulate in the stream until the pipeline is resumed, subject to the stream's `max_bytes` and `max_age` limits. When the pipeline is **terminated**, the streams are deleted and any buffered data is lost. See [OTLP Source — Pipeline Stop and Terminate Behaviour](/sources/otlp#pipeline-stop-and-terminate-behaviour) for details.
...
3. Only unique messages are forwarded to the downstream stream (or sink).
...
1. Messages are read from the upstream NATS stream (ingestor or previous processors in this stage).
