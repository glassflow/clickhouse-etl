...
With the **Dead-Letter Queue (DLQ):** Fault events at ingestion will be logged to the [DLQ](/usage-guide/python-sdk#dead-letter-queue-dlq) without blocking your current pipeline. Giving you the chance to re-run those events and have your ClickHouse complete.

![DLQ metrics panel](/assets/dlq-metrics.png)

### Real-World Data Handling:

GlassFlow is now supporting **Nested JSON.** It allows you to seamlessly ingest, flatten and process complex Kafka event payloads that mirror messy real-world data. Feel free to check out our example [here](/sources/kafka/data-format#nested-record-fields).
