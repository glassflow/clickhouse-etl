...
The docs site now ships a dedicated **Sources / Integrations directory** at [`/sources`](/sources). The page lists every supported source, marks each as Open Source or Enterprise, and links to a per-source guide. Coverage includes 29 sources spanning streaming platforms, telemetry collectors, databases, object storage, and table formats.
...
There are **no breaking changes** in v3.2.0 if you are already on v3.1.0. The behaviour change in the sink is backward-compatible: existing pipelines simply see fewer DLQ entries from transient ClickHouse errors and lower data loss on flaky connections.
...
*GlassFlow v3.2.0 turns the backpressure foundations from the previous release into an end-to-end observable behaviour and meaningfully reduces unnecessary DLQ traffic from transient sink failures.*
