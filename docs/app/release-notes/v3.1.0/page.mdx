...
Every stage that publishes to a JetStream stream now detects backpressure (the downstream stream hitting its `MaxMsgs` cap), retries with bounded backoff, and emits observability data so operators can see episodes as they happen.
...
### 🎚️ Configurable NATS stream cap (`MaxMsgs`)
...
- **Pipeline config.** `resources.nats.stream.maxMsgs` is the new knob. Defaults are computed automatically from the sink batch size, so most pipelines need no manual tuning.
...
2. **Verify the data migration.** Watch for the one-shot `glassflow-etl-data-migration` Job to complete; check Postgres for any pipeline rows whose persisted config is still in the v2 shape.
