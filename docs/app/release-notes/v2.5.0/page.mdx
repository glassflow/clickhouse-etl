...
sidebarTitle: v2.5.0
icon: FileIcon
description: Release notes for the GlassFlow ClickHouse ETL v2.5.0 - Major architectural improvements with PostgreSQL metadata storage and enhanced deduplication.
---

# Release Notes v2.5.0

Version 2.5.0 represents a **major architectural upgrade** to GlassFlow, introducing PostgreSQL-based metadata storage, high-performance file-based deduplication, and a completely redesigned user interface. This release significantly improves scalability, performance, and user experience while maintaining the core functionality that makes GlassFlow powerful.

## 🚀 Migration Required

**Important**: Upgrading to v2.5.0 requires following specific migration steps due to architectural changes.

👉 **[Complete Migration Guide →](/installation/migration/v2.5.0)** - Step-by-step instructions for upgrading from v2.4.x
...
3. **Verify new chart version**:
   ...
   # Should show version: 0.4.0, appVersion: 2.5.0
   ```
...
**⚠️ Important**: Rolling back from v2.5.0 to v2.4.x is **not supported** due to the architectural changes. Ensure you have:
...
To experience the new features in v2.5.0:

1. **Follow the migration guide** to upgrade from v2.4.x or install the version v2.5.0 on a cluster
...
GlassFlow v2.5.0 includes CLI support for local development:
...
For a complete list of all changes, improvements, and bug fixes in v2.5.0, see our [GitHub release v2.5.0](https://github.com/glassflow/clickhouse-etl/releases/tag/v2.5.0).

*GlassFlow v2.5.0 represents a major step forward in making streaming ETL more scalable, performant, and user-friendly for enterprise production environments.*
