...
Set `"format": "avro"` on the Kafka source and provide the schema via one of the two modes below. For full pipeline examples, field type mappings, and troubleshooting, see the [Avro Pipeline Usage](/configuration/avro) guide.

### Avro — Inline schema

Provide the `.avsc` schema as a JSON-escaped string under `schema.file`:
...
Embed the `.proto` IDL source as a string under `schema.file`. Use `schema.message_type` to select the root message (defaults to the first declared message):
