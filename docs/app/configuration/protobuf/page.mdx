...
Provide the `.proto` source text as a string under `schema.file`. GlassFlow compiles it at pipeline-creation time and stores the resulting `FileDescriptorProto`.
...
|-------|------|----------|-------------|
| `file` | string | Yes | Full `.proto` IDL source text. All referenced types must be defined in the same file — `import` is not supported. Required in both inline and Schema Registry modes. |
| `message_type` | string | No | Root message name. If omitted, the first message declared in the file is used. |
...
The `schema.message_type` field does not match any message declared in the `.proto` source. Check capitalisation — Protobuf message names are case-sensitive.
