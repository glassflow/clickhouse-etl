...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `type` | string | Yes | Source type: `"kafka"` or `"otlp.logs"` / `"otlp.traces"` / `"otlp.metrics"`. See [Sources](/sources) |
...
> 💡 **Note**: For a full list of supported protocols and mechanisms, see the [Supported Kafka Connections](/sources/kafka/connections) documentation.
...
| `expression` | string | Yes | Filter expression. See [Filters Transformation](/transformations/filter) documentation. |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `expression` | string | Yes | Expression evaluated against the input JSON event. See [Stateless Transformations](/transformations/stateless-transformation) documentation for more details on the supported transformations. |
...
> 💡 **Note**: You can find more information about the NATS stream configuration parameters in the [NATS Stream Configuration](https://docs.nats.io/nats-concepts/jetstream/streams#configuration) documentation.
