...
Set the **Security Protocol** ([`protocol`](/configuration/pipeline-config-reference#kafka-connection-parameters)) according to your broker configuration:
...
Set one of the following **Authentication Methods** ([`mechanism`](/configuration/pipeline-config-reference#kafka-connection-parameters)):
...
You can disable authentication entirely by using the mechanism `NO_AUTH` (On the UI is called "No authentication"). It is also possible to skip TLS verification by using the `skip_tls_verification` flag.
