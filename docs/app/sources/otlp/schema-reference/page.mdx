...
Each OTLP signal type exposes a fixed set of fields that you can map to ClickHouse columns in the `schema.fields` array. Set `source_id` to match the signal type (e.g., `"otlp.logs"`).
...
| `resource` | map | Resource-level attributes |
| `scope_name` | string | Instrumentation scope name |
| `scope_version` | string | Instrumentation scope version |
| `scope_attributes` | map | Scope-level attributes |
