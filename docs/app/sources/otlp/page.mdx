...
- [Logstash](/sources/logstash). Routed through an OpenTelemetry Collector relay (`tcplog` then `otlphttp`); Logstash itself has no maintained OTLP output.

For OpenTelemetry SDKs and the OpenTelemetry Collector directly, see [Sending Data](/sources/otlp/sending-data).
...
See [Backpressure](/sources/otlp/back-pressure) for how the two backpressure gates work, how to tune them, and how to resolve sustained backpressure.

## Telemetry collectors and forwarders

The OTLP receiver accepts data from any OpenTelemetry-compatible collector or forwarder. The following tools are validated and have specific integration guides:

- [Logstash](/sources/logstash) -- via OpenTelemetry Collector relay (`tcplog` → `otlphttp`)
...
- [Backpressure](/sources/otlp/back-pressure) -- concurrency limits, NATS stream capacity, tuning and scaling
