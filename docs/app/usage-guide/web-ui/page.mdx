...
2. **Join** - Combine data from multiple Kafka sources
3. **Deduplicate and Join** - Both deduplication and joining in a single pipeline
4. **Single Source** - Ingest, filter, transform, and deduplicate data from a single source
...
### Creating a Single Source Pipeline

This section walks through creating a pipeline that ingests, filters, transforms, and deduplicates records from a single source. The example below uses a Kafka source.
<Steps>
...
- **Max Delay Time**: Maximum time to wait before flushing batch (default: 60s)

**Field Mapping**

Map source fields to ClickHouse columns:

- **Source Field**: The field from your data source
...
- **Replicas**: Number of parallel instances. More replicas increase throughput for that stage.
  - For Kafka sources, ingestor replicas must not exceed the topic's partition count
...
### Creating a Multi-Source Pipeline

This section covers creating pipelines that combine data from multiple Kafka sources.
<Steps>
...
| `Running` | Stable | The pipeline is actively consuming from its source, processing data, and writing to ClickHouse. |
