...
## Open Source and Enterprise clients

The package ships two clients. The Open Source client is `from glassflow.etl import Client`. The Enterprise Edition adds `from glassflow.ee import Client`, which extends the Open Source client with DLQ reprocessing, pipeline stream inspection (`get_streams`), and the Avro and Protobuf source formats. The examples below use the Open Source client, and Enterprise-only capabilities are called out where they apply.

Kafka sources declare their fields with `schema_fields` (JSON). Avro and Protobuf sources (Enterprise) carry their schema in the `schema` object instead. See [Data Formats](/sources/kafka/data-format) for the full schema reference.
...
With the Enterprise client (`from glassflow.ee import Client`), `pipeline.dlq` adds non-destructive, paginated reading and reprocessing: `list()` and the `list_iter()` generator (both accept a `component` filter, one of `ingestor`, `join`, `sink`, `dedup`, `otlp-receiver`), plus `reprocess()` / `reprocess_all()` and `discard()` / `discard_all()`.
