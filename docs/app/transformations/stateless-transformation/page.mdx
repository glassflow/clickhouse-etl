---
title: 'Stateless Transformation'
...
3. **Type Conversion**:  
   The result of the expression is converted to the configured `output_type` (string, int, uint, float64, bool, or `[]string`).
4. **Output Assembly**:  
...
<Tabs items={['YAML', 'JSON', 'Web UI']} defaultIndex="2" storageKey="config_format">
  <Tabs.Tab>
    ...
    For the full field reference, see the [Pipeline Configuration Reference](/configuration/pipeline-config-reference#stateless-transformation).
  </Tabs.Tab>
  <Tabs.Tab>
    Stateless transformations use the following structure. The `transforms` array lists every field that appears in the output, both derived fields (with expressions) and passthrough fields (e.g. `expression: "user_id"` to pass `user_id` through unchanged).
